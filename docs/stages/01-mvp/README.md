...
| C | Тимофей временно, основной владелец Никита | Reset, внешние триггеры, доставка событий и realtime |
...
В A3 `POST /api/demo/reset` был передан в C2: ему нужна отдельная транзакция восстановления seed. Реализованный результат C2 описан ниже.
...
- C1: STOMP подключается через `/ws`, единственный исходящий topic — `/topic/progression`.
- Topic передаёт тот же `ProgressionResponse`, что REST, только для `applied=true`.
- Повтор команды не создаёт realtime-событие; ошибка доставки не отменяет успешный REST и транзакцию.
- C2: `POST /api/demo/reset` в одной транзакции восстанавливает точный seed и возвращает `DemoStateResponse`.
- Reset удаляет лишние демо-записи и восстанавливает отсутствующие; перед заменой таблицы блокируются.
- Схема, зависимости и JSON-контракт не изменились.
- Проверка на Java 21 и PostgreSQL 17: 40 тестов, 0 ошибок, включая реальное STOMP-подключение.
- Vue подключает одну STOMP-сессию при старте, подписывается на `/topic/progression` и автоматически переподключается через 3 секунды.
- Типизированный клиент не меняет визуальную часть B; компоненты получают события через `progressionRealtime.subscribe(...)`.
- Живая проверка подтвердила полный путь: браузер подключился и получил событие завершения из backend.

Применение события к экранному состоянию остаётся частью I1 и совместной точкой с потоком B. Инициирующий UI использует REST-ответ, а realtime синхронизирует другие открытые экраны.

## Точки интеграции

- B может подключить два готовых endpoint после A3; `nextUnlock` в TypeScript должен допускать `null`.
- C готов: backend публикует событие, Vue-клиент принимает его; B подключает готовую подписку к состоянию страницы без изменения DTO.
...
Следующий шаг: Андрей заменяет fixture живыми GET/POST вызовами в I1 и применяет события готового STOMP-клиента к тому же состоянию страницы.
