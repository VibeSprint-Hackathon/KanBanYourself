...
| B3 Реакция прогресса | `PARTIAL` | Живая реакция готова; отдельной доски ещё нет |
...
| B3b Минимальная доска | `NEXT` | Показать три контрактных статуса из общего state |
...
## B3b — завершить минимальную доску (`NEXT`)

Показать Квесты из того же `DemoState` в колонках `TODO`, `IN_PROGRESS`, `DONE`. Допустимо встроить компактную доску в Dashboard вместо отдельного маршрута. Она не получает второй store и не меняет статусы локально в обход ответа backend.

## I2 — проверить вертикальный сценарий (`PLANNED`)

После B3b выполнить три цикла `reset -> load -> complete -> repeat -> refresh`. Проверить пропавший WebSocket, быстрый двойной клик, nullable `nextUnlock`, максимальный уровень и reduced motion. Исправлять только воспроизведённые дефекты.
...
Вход следующей задачи: Dashboard полностью работает с живым API и единым store. Выход B3b: все три Квеста показаны по контрактным статусам без второго источника состояния.
