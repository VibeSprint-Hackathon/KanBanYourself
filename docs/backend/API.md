...
| `POST /api/demo/reset` | Не реализован | Будет добавлен Никитой в C2 |
...
Reset пока нельзя вызывать: `POST /api/demo/reset` появится после реализации C2 владельцем потока C.
